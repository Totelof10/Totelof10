...
import { BiSolidUserDetail } from 'react-icons/bi'; // Specific icon for detail page header
    ...
    const fetchCustomer = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setCustomer(null); // Explicitly set to null if not found or other status
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-700">Chargement des détails du client...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
        <div className="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg shadow-md max-w-md text-center">
          <h4 className="text-2xl font-bold mb-2">Client introuvable</h4>
          <p className="text-lg mb-4">Désolé, le client que vous recherchez n'existe pas ou a été supprimé.</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col items-center">
        ...
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-1 flex items-center">
              <BiSolidUserDetail className="text-teal-600 mr-3 text-4xl" /> {/* Adjusted icon color */}
              Détail du client
            </h1>
            <p className="text-gray-600 text-lg md:text-xl">Informations complètes sur le client</p>
          </div>
        </div>
      ...
      <div className="w-full max-w-6xl bg-white shadow-xl rounded-lg overflow-hidden flex flex-col">
        {/* Card Header */}
        <div className="bg-teal-600 text-white p-5 flex items-center rounded-t-lg"> {/* Adjusted color from info to teal */}
          <FaUser className="mr-4 text-4xl" /> {/* Adjusted size */}
          <div>
            <h2 className="text-3xl font-extrabold">{customer.name}</h2>
            <p className="text-teal-100 text-sm">Client</p> {/* Adjusted color for small text */}
          </div>
        </div>

        {/* Card Body with Information Grid */}
        <div className="bg-slate-50 p-6 grid grid-cols-1 md:grid-cols-2 gap-6"> {/* Light background for body */}
          {/* Email */}
          ...
          {/* Contact Person */}
          <div className="bg-white rounded-lg p-4 shadow-sm flex items-center transition-transform hover:scale-105 duration-200">
            <FaUser className="mr-4 text-orange-500 text-3xl" /> {/* Changed color from warning to orange */}
            <div>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm flex items-center transition-transform hover:scale-105 duration-200">
            ...
            <div>
              ...
              <p className="text-gray-600 break-words">{customer.address}</p>
            </div>
          </div>
        </div>
      </div>
